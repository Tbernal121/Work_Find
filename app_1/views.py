...
from .forms import ingresarAspiranteForm, ingresarEmpresaForm, ingresarOfertaForm, PresentarseOfertaForm, HabilidadForm, ArchivoPDFForm, Oferta
...
def delete_view(request, pk):
    oferta = get_object_or_404(Oferta, pk=pk)
    if str(request.user) != oferta.Empresa_id_empresa.Usuario_id_usuario.nombre_usuario:  
        messages.warning(request, "No tienes permiso para eliminar esta oferta")   
        return redirect('ofertas')
        #return HttpResponse("No tienes permiso para eliminar esta oferta")
    if request.method == 'POST':
        oferta.delete()
        return redirect('ofertas')    
    return render(request, 'ofertas.html', {'oferta': oferta})